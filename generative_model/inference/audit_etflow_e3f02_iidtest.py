"""Independent pymatgen audit of the frozen E3/F0.2 IID-test panel."""
...
DEFAULT_INPUT = ROOT / "generative_model/runs/etflow_e3f02_iidtest_v1"
...
def summarize_point_groups(
    ...
    for record in records:
        ...
        success = bool(record["analyzer_success"])
        is_compatible = bool(record["pg_compatible"])
        analyzer_success.append(success)
        compatible.append(is_compatible)
        exact.append(bool(record["pg_exact_match"]))
        by_pg[point_group].append(is_compatible)
        if record["actual_pg"] is not None:
            actual_counts[str(record["actual_pg"])] += 1
    metrics = {
        "molecule_count": len(records),
        "analyzer_success_fraction": float(np.mean(analyzer_success)),
        "pg_compatible_fraction": float(np.mean(compatible)),
        "pg_exact_match_fraction": float(np.mean(exact)),
        "c2_pg_compatible_fraction": float(np.mean(by_pg["C2"])),
        "c3_pg_compatible_fraction": float(np.mean(by_pg["C3"])),
    ...
    checks = {
        ...
        "analyzer_success_fraction_min": (
            metrics["analyzer_success_fraction"]
            >= threshold["analyzer_success_fraction_min"]
        ),
        "pg_compatible_fraction_min": (
            metrics["pg_compatible_fraction"]
            >= threshold["pg_compatible_fraction_min"]
        ),
        "c2_pg_compatible_fraction_min": (
            metrics["c2_pg_compatible_fraction"]
            >= threshold["c2_pg_compatible_fraction_min"]
        ),
        "c3_pg_compatible_fraction_min": (
            metrics["c3_pg_compatible_fraction"]
            >= threshold["c3_pg_compatible_fraction_min"]
        ),
    }
...
def audit(protocol_path: Path, input_dir: Path) -> dict[str, Any]:
    # pymatgen deliberately lives only in env_cof; keep report accounting importable
    # from env_etflow for unit tests and the GPU runner.
    from generative_model.data.symmetry import SymmetryProtocol
    from generative_model.evaluation.symmetry_metrics import (
        analyze_evaluation_symmetry,
    )
    ...
    for progress, package_index in enumerate(
        map(int, protocol["panel"]["package_indices"]), start=1
    ):
        source_path = molecule_dir / f"{package_index:06d}.json"
        ...
        print(json.dumps({
            "audited": progress,
            "total": protocol["panel"]["molecule_count"],
            "package_index": package_index,
            "generation_success": result["generation_success"],
            "actual_pg": result["actual_pg"],
            "compatible": result["pg_compatible"],
        }, ensure_ascii=False), flush=True)

    metrics, checks = summarize_point_groups(
        records, protocol, bool(geometry["passed_geometry_gate"])
    )
    passed = all(checks.values())
    report = {
        "schema_version": "etflow-e3f02-iidtest-point-group-v1",
        "status": (
            protocol["decision"]["final_pass_status"]
