"""
env.py — Original COF construction environment (6-agent vocabulary-based).

Agents sequentially construct Building Blocks from a vocabulary of substructures,
using Transformer-encoded states. Actions select the next substructure token
until the BB is complete, then connectors and functional groups are added.

Interface (Gym-compatible):
    reset() → observations, info, action_mask
    step(actions, episode_num) → obs_next, rewards, dones, info, action_mask

Key attributes:
    self.n = 6           # number of agents
    self.action_space = 53  # vocabulary size
    self.observation_space = 128  # Transformer-encoded state dim

Dependencies: transformer.py (Embedding_Layer, TransformerEncoder), reward.py
"""
