"""
ccjson.py — ChemJSON utility functions.

Reads and processes ChemJSON (.cjson) files for COF building blocks.
Used by combine_substructures.py to extract SMILES and atom labels.

Functions:
    from_cjson(path, file_name) — read properties from a .cjson file
    get_xmiles(path, file_name) — extract XSMILES representation
    get_labels(path, file_name) — extract atom labels
"""
