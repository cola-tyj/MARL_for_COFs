"""
mappo_mpe.py — MAPPO algorithm implementation (Multi-Agent PPO).

Core MAPPO training components:
  - Actor_RNN / Actor_MLP: policy networks for discrete actions
  - Critic_RNN / Critic_MLP: centralized value function
  - MAPPO_MPE: main training class with choose_action(), get_value(), train()

Supports:
  - Orthogonal initialization (Trick 8)
  - GAE advantage computation
  - Value clipping
  - Learning rate decay
  - RNN (GRU) or MLP actor/critic
...
class Actor_MLP(nn.Module):
    ...
    def forward(self, actor_input, action_mask=None):
        # When 'choose_action': actor_input.shape=(N, actor_input_dim), prob.shape=(N, action_dim)
        ...
        x = actor_input
        x = self.activate_func(self.fc1(x))   # N x 128 -> N x 128
        x = self.activate_func(self.fc2(x))   # N x 128 -> N x 128
        prob = torch.softmax(self.fc3(x), dim=-1)
        if action_mask is not None:
            action_mask = torch.BoolTensor(action_mask)
            prob = prob.masked_fill(~action_mask, float(0.0))
        return prob
...
class MAPPO_MPE:
    ...
    def choose_action(self, obs_n, action_mask, evaluate):
        with torch.no_grad():
            actor_inputs = []
            ...
            if self.add_agent_id:
                """
                    ...
                    [obs of agent_2]+[0,1,0]0
                    [obs of agent_3]+[0,0,1]
                    So, we need to concatenate a N*N unit matrix(torch.eye(N))
                """
                actor_inputs.append(torch.eye(self.N))

            actor_inputs = torch.cat([x for x in actor_inputs], dim=-1)  # actor_inputs.shape=(N, actor_input_dim)
            actor_inputs = obs_n
            #print('actor')
            prob = self.actor(actor_inputs, action_mask)  # prob.shape=(N,action_dim)
    ...
    def get_value(self, s):
        with torch.no_grad():
            ...
            s = s.unsqueeze(0).repeat(self.N, 1)  # (state_dim,)-->(N,state_dim)
