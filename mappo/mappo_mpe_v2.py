"""
mappo_mpe_v2.py — MAPPO v2 with MultiDiscrete action space support.

Extends MAPPO_MPE for environments with multiple discrete action heads
per agent (e.g., symmetry + core selection as separate actions).

Key additions over mappo_mpe.py:
  - MultiDiscrete action space handling
  - Per-head action masking
  - Multi-head policy evaluation
"""
