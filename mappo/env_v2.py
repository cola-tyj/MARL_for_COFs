"""
env_v2.py — COF Design Environment v2 (3-agent, diffusion-integrated).

Agents design COFs by selecting symmetries and topologies, with the
diffusion model generating the actual Core structures. Supports
coord-only refinement for 100% atom preservation.

Differs from env.py: uses symmetry+topology instead of vocabulary-based
construction. Agents select from fixed action spaces rather than tokens.
"""
