...
def get_unique_atoms(atoms):
    """
    找出输入中的不同原子种类数,以及重复次数
    具有相同邻居原子(被认为处于相同的拓扑中)的原子被视为相同的
    ...
    """
    # get graph
    cutoff = natural_cutoffs(atoms)  # 获取原子间成键
    neighbor_list = neighborlist.NeighborList(
        cutoff, self_interaction=True, bothways=True
    )
    neighbor_list.update(atoms)
    matrix = neighbor_list.get_connectivity_matrix()  # 获取连通矩阵
...
def get_crystal_graph(atoms, radius=8, max_num_nbr=12):
    """
    输入ase.io.read(CIF)输出图数据
    将最近的max_num_nbr个原子视为有连接

    Args:
        atoms : _description_
        max_num_nbr (int, optional): 最大邻接原子数. Defaults to 12 by CGCNN.

    Returns:
        atom_num: [N]原子核电荷数
        nbr_idx: [12*N]和原子具有连接的其他原子下标
        nbr_dist: [12*N]存在连接的原子间的距离
        uni_idx: [U, x]所有原子被划分为U种类型,每种类型包含一系列原子下标
        uni_count: [U, 1]每种类型的原子数
    """
    ...
    nbr_dist = []
    for row in nbr_mat:
        idx = np.argsort(row)[:max_num_nbr]  # 和当前原子有连接的原子下标
        nbr_idx.extend(idx)  # 记录下标
        nbr_dist.extend(row[idx])  # 记录距离

    # get same-topo atoms,将晶体中的所有原子分到拓扑结构里,并计算原子的重复次数
...
def _make_supercell(atoms, cutoff):
    ...
    Args:
        atoms : 使用ASE从CIF读取的晶体
        cutoff : 截断半径
    """
    scale_abc = []
    # 读取atom的晶格参数,如果小于截断半径,则放大为>=截断半径的大小
    ...
    atoms = make_supercell(atoms, m)  # m是对角矩阵,对应晶格参数的放大倍数
    return atoms
...
def make_prepared_data(
    ...
    max_length = kwargs.get("max_length", 659.0)  # !!!!! 晶格最大长度,目前设置为coffee的最大值
    min_length = kwargs.get("min_length", 3.0)  # !!!!! 晶格最小长度,目前设置为coffee的最小值
    max_num_nbr = kwargs.get("max_num_nbr", 12)  # 一个原子连接其他原子的最大个数（单个原子连接的最大个数）
    max_num_unique_atoms = kwargs.get("max_num_unique_atoms", 300)
    max_num_atoms = kwargs.get("max_num_atoms", None)  # 最大原子数,默认无限
    ...
    # 1. 创建超晶胞,如果晶格参数小于指定的截断半径,则放大一定的倍数至超过该值
