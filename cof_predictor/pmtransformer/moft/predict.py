...
from moftutils.validation import (
...
def predict(root_dataset, load_path, downstream=None, split='all', save_dir=None, graphEmbedSavePath=None, 
            **kwargs):
    """
    ...
    config['graph_embed_save_path'] = graphEmbedSavePath
...
def main(_config):
    ...
    os.makedirs(config["log_dir"], exist_ok=True)  # exist_ok=True 如果文件夹已存在不报错
    pl.seed_everything(config['seed'])  # pytorch lightning设置随机数种子
    ...
    config = get_valid_config(config)  # 读取验证的配置

    # 使用pytorch lightning构造模型和数据模块
    model = Module(config)
    ...
    trainer = pl.Trainer(
        accelerator=config['accelerator'],  # 指定设备类型,config['accelerator']
        devices=config["devices"],  # config["devices"]
        num_nodes=1,  # 指定机器数,config["num_nodes"]
        precision=config["precision"],  # 指定精度,fp16可以选择更大的batch
        strategy=None,  # 训练策略dp/ddp...
        benchmark=True,  # True算法选择和内存使用等方面采取更激进的优化策略,输入大小不变的情况下可以加速
        max_epochs=1,
        log_every_n_steps=0,
        deterministic=True,  # 结果可复现
        logger=False,
    )

    # 选择预测数据集,train/val/test,默认all
    ...
    for s in split:
        ...
        savefile = save_dir/f'{s}_{downstream}_prediction.csv'
        dataloader = getattr(dm, f'{s}_dataloader')()  # 获取对应集合的数据加载器
        rets = trainer.predict(model, dataloader)  # list(71,每个输入对应一个dict)->dict(3个字段:cif_id,val,label)
