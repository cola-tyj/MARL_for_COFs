...
class QM9Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Data:
        ...
        # One-hot atom types (fully vectorized, no Python loop)
        x = torch.zeros(n, QM9_NUM_ATOM_TYPES)
        # Map atomic numbers to indices via tensorized lookup
        atom_idx_map = torch.tensor(
            [QM9_ATOM_TO_IDX.get(an, 1) for an in QM9_ATOMIC_NUMBERS]
        )  # Length 5: H→0, C→1, N→2, O→3, F→4
        # Create a reverse mapping: atomic_number → atom_type_index
        # For QM9, atomic numbers are in [1,6,7,8,9], use searchsorted
        z_tensor = torch.tensor([1, 6, 7, 8, 9])  # all possible QM9 atomic numbers
        # For each atom, find its index in z_tensor
        col_idx = torch.searchsorted(z_tensor, z)
        col_idx = col_idx.clamp(0, QM9_NUM_ATOM_TYPES - 1)
        x[torch.arange(n), col_idx] = 1.0
    ...
    def _infer_bonds(
        ...
        # Map atomic numbers to radii without Python loops
        # Known atomic numbers: 1(H),6(C),7(N),8(O),9(F)
        radii_map = torch.tensor([_COVALENT_RADII.get(z, 0.7) for z in [1, 6, 7, 8, 9]],
                                 dtype=torch.float32)
        # For each edge, get atom_z of source and target
        z_row = z[row]  # (E,) atomic numbers of source atoms
        z_col = z[col]  # (E,) atomic numbers of target atoms
        # Map atomic numbers 1,6,7,8,9 → indices 0,1,2,3,4
        z_idx = torch.searchsorted(torch.tensor([1, 6, 7, 8, 9]),
                                    torch.stack([z_row, z_col], dim=-1))
        z_idx = z_idx.clamp(0, 4)
        r_i = radii_map[z_idx[:, 0]]
        r_j = radii_map[z_idx[:, 1]]
        r_sum = r_i + r_j
