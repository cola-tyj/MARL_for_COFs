...
# Grouped by symmetry order — synchronized with point_group.py CANDIDATE_POINT_GROUPS
MOLECULAR_POINT_GROUPS = [
    ...
    "D2d",   # 3 C2 + 2 dihedral mirrors (S4 improper axis)
    "Td",    # Tetrahedral (4 C3 + 3 C2 + 6 mirrors)
]
...
def build_point_group_binary_matrix() -> torch.Tensor:
    ...
    for pg_name, pg_idx in POINT_GROUP_TO_IDX.items():
        ...
        if pg_name == "C1":
            ...
            pg_vec[0, 0] = 1.0
            pg_vec[0, 2] = 1.0  # C2 on principal
            pg_vec[0, 8] = 1.0  # S4 rotoinversion (improper C4)
            pg_vec[1, 0] = 1.0
            pg_vec[1, 2] = 1.0  # C2 on secondary
            pg_vec[1, 6] = 1.0  # sigma_d
            pg_vec[2, 0] = 1.0
            pg_vec[2, 2] = 1.0  # C2 on tertiary
            pg_vec[2, 6] = 1.0  # sigma_d'

        elif pg_name == "Td":
            # Tetrahedral: 4 C3 axes (body diagonals), 3 S4 axes (x,y,z),
            # 6 mirror planes. Encoded across all 3 principal axes.
            pg_vec[0, 0] = 1.0
            pg_vec[0, 3] = 1.0  # C3 on body diagonals (principal component)
            pg_vec[0, 8] = 1.0  # S4 on principal
            pg_vec[1, 0] = 1.0
            pg_vec[1, 3] = 1.0  # C3 (secondary component)
            pg_vec[1, 8] = 1.0  # S4 on secondary
            pg_vec[1, 6] = 1.0  # sigma
            pg_vec[2, 0] = 1.0
            pg_vec[2, 3] = 1.0  # C3 (tertiary component)
            pg_vec[2, 8] = 1.0  # S4 on tertiary
            pg_vec[2, 6] = 1.0  # sigma
