...
> **已停止的历史计划**：本文中的 2/3/6-Agent 与 symmetry-conditioned diffusion 集成方案均不是当前任务。
...
核心发现：QM9 模型和 Phase 2 Core 模型有相同的限制——无法从纯噪声采样，需要模板+精修。这在论文中是合理的 limitation（diffusion refinement 而非 de novo generation），可以讨论。
...
构建一个 **3-Agent MARL 系统**，流水线如下：

```
Agent 决策          扩散模型生成       pycofbuilder 组装       COF 评估
(高层离散)         (连续几何)        (确定性化学)           (吸附模拟)
┌──────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────┐
│ 拓扑+堆叠  │    │ 对称性条件    │    │ Core          │    │ COF 晶体  │
│ Core-A规格 │───→│ 扩散模型      │───→│ + Connector   │───→│ N₂吸附量  │
│ Core-B规格 │    │ → Core(Q+R)  │    │ + FuncGroup   │    │ → Reward  │
└──────────┘    └──────────────┘    └──────────────┘    └──────────┘
                                    确定性化学组装         GCMC 模拟
```

- **Agent**：选择高层设计参数（拓扑类型、堆叠模式、对称性规格）
- **对称性条件扩散模型**：根据对称性条件生成 **Core**（刚性分子骨架 + Q/R 连接点），而非完整 Building Block
- **pycofbuilder**：确定性化学组装（Core + Connector + Functional Group → 完整 BB → COF 晶体）
- **Self-Play 闭环**：高质量 COF → 提取其 Core → 加入扩散模型训练集

### 1.3 核心创新点

1. 将 SymmCD 的**对称性二进制编码**从晶体空间群（230 种）适配到分子点群（16 种：C1~Td）
2. **Core-only 生成策略**：扩散模型只生成刚性骨架（Core），pycofbuilder 确定性组装完整构件——分离几何生成与化学组装
3. 混合**连续扩散**（原子坐标）+ **离散扩散**（原子类型 + 键类型）
4. MARL 从离散选择（53 动作）升级为**连续生成**（扩散模型在对称性条件下采样 Core）
...
```
改造前（固定词汇表）:              改造后（扩散生成 Core）:
┌──────────────────────┐         ┌──────────────────────────────────────┐
│ 6 个 Agent             │         │ 2 个 Agent                            │
│ 每步选 1 个 token       │         │ 一次性指定完整设计                     │
│ 从 53 个离散动作        │         │                                        │
│ 组合优化               │         │ Agent 1: Core-A 对称性+模板             │
│ 仅重组已知构件          │         │ Agent 2: Core-B 对称性+模板             │
│                        │         │         ↓                              │
│                        │         │ 拓扑由对称性对自动确定(!)                │
│                        │         │         ↓                              │
│                        │         │ 扩散模型生成 Core(Q+R)                   │
│                        │         │         ↓                              │
│                        │         │ pycofbuilder 确定性组装 BB              │
└──────────────────────┘         └──────────────────────────────────────┘

关键改进：拓扑由 (conn_a, conn_b) 自动推导，避免 ~28% 无效组合。
```
...
| 文件 | 核心类 | 功能 |
|------|--------|------|
| `marl_interface/env_v2.py` | `COFDesignEnv` | 2-Agent 环境，拓扑自动推导 |
| `marl_interface/train_mappo_cof.py` | `PPOAgent` | PPO 训练脚本 (2000 episodes) |
| `marl_interface/diffusion_env.py` | `DiffusionGeneratorWrapper` | 扩散 Core Pool 缓存 + COF 组装 |
...
```python
# 真实 N₂ reward（40 COF 预计算查找表）
R = N₂_adsorption × 10.0 + 5.0

# Mock 奖励（fallback）
R = base_adsorption(topology) + size_bonus + symmetry_bonus + noise

# 不合法拓扑-对称性组合：即时返回 -1（跳过 pycofbuilder, <1ms）
# 组装失败：-1
```
...
| 实验 | Agent 设计 | 拓扑选择 | 说明 |
|:---:|------|------|------|
| A | **3-Agent** | Agent 0 选拓扑 | 当前实现，action masking 保证合法性 |
| B | 2-Agent + topology head | Agent 1 附带选拓扑 | 已完成 |
| C | 2-Agent + random topo | 系统随机选兼容拓扑 | 待实现，检验拓扑选择是否需要学习 |
| D | 单Agent 预验证组合 | 扁平动作空间 | v1 baseline |

对比指标：COF 组装成功率、平均 N₂ 吸附值、训练收敛速度、最佳设计多样性。
